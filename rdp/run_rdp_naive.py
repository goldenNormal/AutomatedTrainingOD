...
class RDPTree():
    ...
    def training_process(self,
        ...
        return roc_auc_score(labels_ori, scores) ,train_time,scores
                        ...
                        for dropout in dropouts:
                            for filter_ratio in filter_ratios:
                                for epoch in epochs:
                                    ...
                                    Auc,rt,outlier_score = tree.training_process(
                                            x=x,
                                            labels=y,
                                    ...
                                    np.savetxt(f'../eval_uoms/prediction/{template_model_name}/{file_name}/{out_c}-{lr}-{dropout}-{filter_ratio}-{epoch}-{seed}.txt', outlier_score)

                                    AUC.append(Auc)
                                    RT.append(rt)
                pd.DataFrame({'AUC': AUC, 'Time': RT}).to_csv(f'./rdp-nai-{file_name}-{seed}.csv', index=False)
